...
class Client {
   ...
   public:
    ...
    void StartClient(short port) {
        ...
        if (connect(this->clientSock, (struct sockaddr*)&clientAddress, socklen) < 0) {
    ...
    void send_command() {
        ...
        while (true) {
            ...
            try {
                client.massage = std::stoi(input);
            } catch (const std::exception& e) {
                std::cout << "Ошибка! Введите целое число.\n";
                continue;
            }

            if (send(this->clientSock, &client.massage, sizeof(client.massage), 0) < 0) {
    ...
    int recv_command() {
        ...
        int bite_size = recv(this->clientSock, &client.massage, sizeof(client.massage), 0);
        ...
        if (bite_size == 0) {
            ...
            std::cerr << "Возникла ошибка при принятии данных от сервера" << strerror(errno);
            exit(1);
        }
        return 1;
    }
